...
class ParkAttendanceService {
   ...
   protected static viewParkAttendancesService = async () => {
      try {
         ...
         results.forEach((att) => {
            const uniqueId = att.user?.unique_id?.toString() ?? att.user?.unique_id ?? "UNKNOWN_USER";
         ...
         const summaries = Object.values(grouped).map((records: any[]) => {
            ...
            records.sort((a, b) => new Date(a.entry_time || a.createdAt).getTime() - new Date(b.entry_time || b.createdAt).getTime());
            
            const attendanceTimes: Array<{type: string, time: string, datetime: any}> = [];
            let inCount = 0;
            let outCount = 0;
